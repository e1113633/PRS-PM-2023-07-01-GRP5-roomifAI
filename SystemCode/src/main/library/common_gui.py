...
def check_if_model_exist(
    ...
    if save_model_as in ['diffusers', 'diffusers_safetendors']:
        ...
        if os.path.isdir(ckpt_folder):
            msg = f'A diffuser model with the same name {ckpt_folder} already exists. Overwriting file.'
    elif save_model_as in ['ckpt', 'safetensors']:
        ckpt_file = os.path.join(output_dir, output_name + '.' + save_model_as)
        if os.path.isfile(ckpt_file):
            msg = f'A model with the same file name {ckpt_file} already exists. Overwriting file.'
    else:
        log.info(
