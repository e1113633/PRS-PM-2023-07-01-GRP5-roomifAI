...
def load_control_net(v2, unet, model):
  ...
  ctrl_unet_du_sd = model_util.convert_ldm_unet_checkpoint(v2, ctrl_unet_sd_sd, unet_config)    # DiffUsers version of ControlNet state dict
  ...
  # Create a ControlNet other than U-Net
  ctrl_net = ControlNet()
...
def get_guided_hints(control_nets: List[ControlNetInfo], num_latent_input, b_size, hints):
  ...
  for i, cnet_info in enumerate(control_nets):
    # The hints should be arranged as cnet1 in the first image, cnet2 in the first image, cnet3 in the first image, cnet1 in the second image, cnet2 in the second image, etc.
    b_hints = []
    if len(hints) == 1:  
       # Use all the same images as hints        
      hint = hints[0]
...
def call_unet_and_control_net(step, num_latent_input, original_unet, control_nets: List[ControlNetInfo], guided_hints, current_ratio, sample, timestep, encoder_hidden_states):
  # ControlNet
  # For multiple ControlNets, apply outputs alternately instead of merging
