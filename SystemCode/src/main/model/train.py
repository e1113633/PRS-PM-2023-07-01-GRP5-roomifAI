...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def process_caption(self, subset: BaseSubset, caption):
        ...
        if is_drop_out:
            ...
            # textual inversion correspondence
    ...
    def get_input_ids(self, caption):
        ...
        if self.tokenizer_max_length > self.tokenizer.model_max_length:
            ...
            if self.tokenizer.pad_token_id == self.tokenizer.eos_token_id:
                # v1
                # If tokens are of length 77 or above, the total is 227 with "<BOS> .... <EOS> <EOS> <EOS>"
                for i in range(
                ...
                for i in range(
                    ...
                    # If the ending is x <PAD/EOS>, change the ending to <EOS> (if it is x <EOS>, there will be no change)
    ...
    def make_buckets(self):
        """
        Required to call even if bucketing is not performed (create only one bucket)
        ...
        # Create a bucket and distribute images to the bucket
        if self.enable_bucket:
            if self.bucket_manager is None:  
                # In case of fine tuning, if there is a definition in metadata, it has already been initialized.
        ...
        # Create an index for data reference. This index is used to shuffle the dataset
                ...
                if os.path.isfile(cap_path):
                    with open(cap_path, "rt", encoding="utf-8") as f:
                        try:
                            ...
                            print(f"illegal char in file (not UTF-8)")
                            raise e
                        assert len(lines) > 0, f"caption file is empty"
    ...
    def __init__(
        ...
        for subset in subsets:
            ...
            if os.path.exists(subset.metadata_file):
                ...
                raise ValueError(f"no metadata")
            ...
            for image_key, img_md in metadata.items():
                if os.path.exists(image_key):
                    ...
                    if os.path.exists(npz_path):
                        ...
                        assert len(abs_path) >= 1, f"no image"
                        abs_path = abs_path[0]
        ...
        # else:
        #   print("npz files are not used with color_aug and/or random_crop")
        ...
        if sizes is None:
            ...
            # Initialize bucket information, do not recreate it with make_buckets
            self.bucket_manager = BucketManager(False, None, None, None, None)
            self.bucket_manager.set_predefined_resos(resos)

        # Keep npz information clean
...
def debug_dataset(train_dataset, show_input_ids=False):
    print(f"Total dataset length (steps)")
...
def read_config_from_file(args: argparse.Namespace, parser: argparse.ArgumentParser):
    ...
    if args.output_config:
        ...
        if os.path.exists(config_path):
            print(f"Config file already exists. Aborting...")
            exit(1)
...
def get_optimizer(args, trainable_params):
    ...
    if optimizer_type == "AdamW8bit".lower():
        ...
        if actual_lr <= 0.1:
            print(
                f"learning rate is too low. If using dadaptation, set learning rate around 1.0"
            )
        ...
        # Check the arguments and correct them as appropriate
        ...
        if optimizer_kwargs["relative_step"]:
            ...
            # Processing when trainable_params is group
            if type(trainable_params) == list and type(trainable_params[0]) == dict:
                has_group_lr = False
                for group in trainable_params:
                    p = group.pop("lr", None)
                ...
                if has_group_lr:
                    # Disable args just in case TODO This is not very desirable as the dependencies are reversed.
...
def get_hidden_states(args: argparse.Namespace, input_ids, tokenizer, text_encoder, weight_dtype=None):
    ...
    if args.max_token_length is not None:
        if args.v2:
            # v2: Return the triplet of <BOS>...<EOS> <PAD> ... to <BOS>...<EOS> <PAD>
            states_list = [encoder_hidden_states[:, 0].unsqueeze(1)]  # <BOS>
            for i in range(1, args.max_token_length, tokenizer.model_max_length):
                chunk = encoder_hidden_states[:, i : i + tokenizer.model_max_length - 2]  # After <BOS> until before the last
                if i > 0:
                    for j in range(len(chunk)):
                        if input_ids[j, 1] == tokenizer.eos_token:  # Empty, i.e. <BOS> <EOS> <PAD> ... pattern
                            chunk[j, 0] = chunk[j, 1]  # Copy the value of the next <PAD>
                states_list.append(chunk)  # After <BOS> and before <EOS>
            states_list.append(encoder_hidden_states[:, -1].unsqueeze(1))  # Either <EOS> or <PAD>
            encoder_hidden_states = torch.cat(states_list, dim=1)
        else:
            # v1: Return triplet of <BOS>...<EOS> to <BOS>...<EOS>
            states_list = [encoder_hidden_states[:, 0].unsqueeze(1)]  # <BOS>
            for i in range(1, args.max_token_length, tokenizer.model_max_length):
                states_list.append(encoder_hidden_states[:, i : i + tokenizer.model_max_length - 2])  # After <BOS> and before <EOS>
...
def sample_images(
    ...
    """
    Modified version of StableDiffusionLongPromptWeightingPipeline, so it supports clip skip and prompt weighting.
    """
    ...
    if not os.path.isfile(args.sample_prompts):
        print(f"Missing prompt file: {args.sample_prompts}")
        return
    ...
    if args.sample_sampler == "euler" or args.sample_sampler == "k_euler":
