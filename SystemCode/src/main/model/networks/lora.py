...
# https://github.com/Linaqruf/kohya-trainer/blob/main/networks/lora.py

# Guided by explanation in this presentation: https://www.youtube.com/watch?v=PXWYUTMt-AU
...
class LoRAModule(torch.nn.Module):
    """
    replaces forward method of the original LoraInjectedLinear
    """
    ...
    def merge_to(self, sd, dtype, device):
        ...
        # extract weight from original module, org_module
...
class LoRAInferenceModule(LoRAModule):
    def __init__(self, lora_name, org_module: torch.nn.Module, multiplier=1.0, lora_dim=4, alpha=1):
...
def create_network(multiplier, network_dim, network_alpha, vae, text_encoder, unet, **kwargs):
    ...
    # If any of the above is specified, enable dim (rank) for each block.
...
def get_block_dims_and_alphas(
    ...
    # conv_block_dims and conv_block_alphas only if specified. If not specified, use conv_dim and conv_alpha
...
# Define the multiplier for the learning rate for each layer for layered learning rate, taking into account the possibility of being called externally.
def get_block_lr_weight(
    down_lr_weight, mid_lr_weight, up_lr_weight, zero_threshold
) -> Tuple[List[float], List[float], List[float]]:
    # If no parameters are specified, do nothing and behave as before.
    if up_lr_weight is None and mid_lr_weight is None and down_lr_weight is None:
        return None, None, None

    max_len = LoRANetwork.NUM_OF_BLOCKS  # Number of up and down layers equivalent to full model
...
def get_block_index(lora_name: str) -> int:
    ...
    if m:
        ...
        if g[0] == "down":
            block_idx = 1 + idx  # There is no LoRA corresponding to 0.
        elif g[0] == "up":
...
def create_network_from_weights(multiplier, file, vae, text_encoder, unet, weights_sd=None, for_inference=False, **kwargs):
    ...
    module_class = LoRAInferenceModule if for_inference else LoRAModule
...
class LoRANetwork(torch.nn.Module):
    NUM_OF_BLOCKS = 12  # Number of up and down layers equivalent to full model
    ...
    def __init__(
        ...
        """
        LoRA network: There are a lot of arguments, but the pattern is as follows
        1. Specify lora_dim and alpha
        2. Specify lora_dim, alpha, conv_lora_dim, conv_alpha
        3. Specify block_dims and block_alphas: Not applicable to Conv2d3x3
        4. Specify block_dims, block_alphas, conv_block_dims, conv_block_alphas: Also applies to Conv2d3x3
        5. Specify modules_dim and modules_alpha (for inference)
        """
        super().__init__()
        self.multiplier = multiplier
    ...
    # Define the multiplier for the learning rate per layer for layered learning rate
    ...
    def prepare_optimizer_params(self, text_encoder_lr, unet_lr, default_lr):
        ...
        if self.unet_loras:
            if self.block_lr:
                # classify lora for each block so that its learning rate can be graphed.
                block_idx_to_lora = {}
    ...
    def save_weights(self, file, dtype, metadata):
        ...
        if os.path.splitext(file)[1] == ".safetensors":
            ...
            import model.train as train_util
